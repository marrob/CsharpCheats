...
namespace Konvolucio.Cheat
{
    ...
    class String_Formatting
    {
        ...
        [Test]
        public void float_string_format()
        {
            float f = 0.001F;

            Assert.AreEqual("0.00", f.ToString("N"));
            Assert.AreEqual("0",f.ToString("N0"));
            Assert.AreEqual("0.001", f.ToString("N3"));


            int d = 12;

            Assert.AreEqual("00C", d.ToString("X3"));

        }

        [Test]
        public void PadingLeftRight()
        {
            //->             hello<-
            //->hello             <-
